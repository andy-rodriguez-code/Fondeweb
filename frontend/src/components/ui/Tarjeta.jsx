...
const TONOS = {
  '': '',
  tinta: 'bg-ink border-ink [&_h2]:text-white [&_p]:text-ink-bright',
}
...
export default function Tarjeta({ as, href, relleno = 'base', tono = '', pie, className = '', children, ...rest }) {
  ...
  const clases =
    `tarjeta flex flex-col border border-border rounded-md bg-white ` +
    `transition-[border-color,box-shadow,transform] duration-[180ms] ` +
    `[&_h3]:mb-2 [&_h4]:mb-2 [&_p]:text-text [&_p]:text-[0.95rem] ` +
